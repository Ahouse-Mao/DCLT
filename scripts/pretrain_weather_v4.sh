...
# 参数集合（按需修改）
epochs_list=(80)
tau_temp=(0.5 0.8 0.3)
patch_lens=(16 32)
patch_strides=(1 2 4)
max_train_lengths=(336 600)
repr_dims_list=(256 512)
pred_lens=(96 336)

mkdir -p logs/LongForecasting

# Helper: run a command but continue the outer loop if it fails.
# It temporarily disables errexit so the caller script (which keeps set -e) won't exit.
run_and_continue() {
    local cmd="$*"
    echo ">>> RUNNING: $cmd"
    ...
    set -e
    if [ $rc -ne 0 ]; then
        echo "$(date +'%Y-%m-%d %H:%M:%S') [warn] command failed (exit $rc): $cmd" >> logs/LongForecasting/errors.log
    fi
    return $rc
}
                ...
                for patch_stride in "${patch_strides[@]}"; do
                    for max_train_len in "${max_train_lengths[@]}"; do
                        for repr_dims in "${repr_dims_list[@]}"; do
                            ...
                            TS=$(date +%d_%H_%M)
                            run_and_continue python -u ./DCLT_main_pretrain_v4.py \
                                --dataset "$dataset" \
                                --epochs "$epoch" \
                                --tau_temp "$tau" \
                                --patch_len "$patch_len" \
                            ...
                            for pred_len in "${pred_lens[@]}"; do
                                ...
                                run_and_continue python -u ./PatchTST_supervised/run_longExp_v4.py \

                        ...
                        for repr_dims in "${repr_dims_list[@]}"; do
                            ...
                            for i in $(seq 1 5); do
                                ...
                                for pred_len in "${pred_lens[@]}"; do
                                    ...
                                    logfile="./checkpoints/${model_id_name}/${TS}_${epoch}_${patch_len}_${patch_stride}_${max_train_len}_${repr_dims}/${model_name}_${TS}_${model_id}_16_8_${repr_dims}_第${i}次.log"

                                    # 运行评估并自动跳过已有日志
                                    # run_and_continue_atomic "${logfile}" 
                                    mkdir -p "$(dirname "${logfile}")"
                                    python -u ./PatchTST_supervised/run_longExp_v4.py \
                                        --pretrain_d_model "${repr_dims}" \
                            ...
                            for i in {1..3}; do
                                for pred_len in "${pred_lens[@]}"; do
                                    model_id="${model_id_name}_${seq_len}_${pred_len}"
                                    logfile="./checkpoints/${model_id_name}/${TS}_${epoch}_${patch_len}_${patch_stride}_${max_train_len}_${repr_dims}/${model_name}_${TS}_${model_id}_16_2_${repr_dims}_第${i}次.log"
                                    ...
                                    python -u ./PatchTST_supervised/run_longExp_v4.py \
                                        ...
                                        --train_epochs 100 \
                                        --patience 20 \
                                        --itr 1 --batch_size 32 --learning_rate 0.0001 >"${logfile}" 2>&1
                                    rc_pretrain_eval=$?
                                    if [[ ${rc_pretrain_eval} -ne 0 ]]; then
                                        echo -e "\n=== FAILED with exit code ${rc_pretrain_eval} ===" >>"${logfile}"
                                    fi

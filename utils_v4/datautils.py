...
import pandas as pd
import torch
from utils_v4.utils import pkl_load, pad_nan_to_target, split_with_nan
...
def load_forecast_csv_patch(name, max_train_length, univar=False):
    data = pd.read_csv(f'dataset/{name}.csv', index_col='date', parse_dates=True)
    ...
    if univar:
        if name in ('ETTh1', 'ETTh2', 'ETTm1', 'ETTm2'):
            ...
            target_col = 'MT_001' if 'MT_001' in data.columns else data.columns[0]
